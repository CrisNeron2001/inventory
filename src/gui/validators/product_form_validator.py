class FormProductValidator:
    ...
    @staticmethod
    def validate_step_product_form_data(
        step_name: str, data: dict
    ) -> tuple[bool, list[str]]:
        if step_name == "basic_data":
            return FormProductValidator.validate_basic_data(data)
        elif step_name == "category_brand":
            return FormProductValidator.validate_category_brand_data(data)
        else:
            return True, []

    @staticmethod
    def validate_category_brand_data(data: dict) -> tuple[bool, list[str]]:
        errors = []
    ...
    def validate_edit_product_form_data(data: dict) -> tuple[bool, list[str]]:
        ...
        if validator:
            ...
            validator.validate_category_brand_data(data)
        return True, []

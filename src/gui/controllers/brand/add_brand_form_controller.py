...
class AddBrandFormController:
    ...
    def on_submit(self, form_data: dict):
        ...
        if not is_valid:
            log.error(
                f"[AddBrandFormController.on_submit] Error al validar el formulario: {errors}"
            )
            self.show_validate_error_dialog(errors)
            return

        brand_dto = BrandDTO(brand_id=autoincrement_id(), name=form_data["name"])

        brand_added = self.brand_service.create_brand(brand_dto)
        if brand_added:
            log.info(
                f"[AddBrandFormController.on_submit] Marca creada: {getattr(brand_added, 'name', '')}."
            )
            self.show_success_dialog(
                f"Marca editada: {getattr(brand_added, 'name', '')}."
            )
            return brand_added
        else:
            log.error(
                "[AddBrandFormController.on_submit] No se pudo crear la marca. Intente nuevamente."
            )
            return self.show_validate_error_dialog(
                ["No se pudo crear una nueva marca. Intente nuevamente."]
            )

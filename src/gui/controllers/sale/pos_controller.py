...
class POSController:
        ...
        def on_add(e: ft.ControlEvent):
            try:
                ...
                if hasattr(e, 'page') and e.page:
                    page = e.page
                    page.snack_bar = ft.SnackBar(ft.Text("Error al agregar al carrito"))
                    page.update()

        def on_remove(index: int):
            try:
                cid = getattr(self, 'current_cart_id', None)
                if cid is not None and 0 <= index < len(self.cart):
                    prod = self.cart[index].get('product')
                    pid = getattr(prod, 'product_id', None)
                    if pid is not None:
                        try:
                            cp_dto = CartProductDTO(cart_id=cid, product_id=pid, quantity=0, cart=None, product=None)
    ...
    def _update_content_internal(self):
        ...
        if isinstance(current, SaleSummaryStep):
            try:
                cid = getattr(self, 'current_cart_id', None)
                # Resetear el carrito en memoria y cargar lo persistido (si existe)
                self.cart = []
                if cid is not None:
                    persisted = self.cart_service.get_cart_by_id(int(cid)) or []
                else:
                ...
                log.error("Error cargando carrito persistido para el resumen", exc_info=True)

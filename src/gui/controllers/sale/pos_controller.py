...
class POSController:
    ...
    def add_to_cart(self, product_id: int, qty: int):
        if self.user_inv_id is None:
            self.show_validate_error_dialog(
                ["Debe iniciar sesión para agregar al carrito."]
            )
            return

        return self.cart_manager.add_to_cart(product_id, self.user_inv_id, qty)
    ...
    def confirm_sale(
        ...
        log.info(
            f"[POSController.confirm_sale] Intentando confirmar venta con payment_method={payment_method} y payment_amount={payment_amount}"
        )
        ...
        if self.user_inv_id is None:
            ...
            log.error(
                "[POSController.confirm_sale] Intento de confirmar venta sin usuario autenticado."
            )
            return None

        log.info(
            f"[POSController.confirm_sale] Usuario autenticado: user_inv_id={self.user_inv_id}"
        )
        log.info(
            f"[POSController.confirm_sale] Confirmar payment_amount={payment_amount} y payment_method={payment_method}"
        )

        return self.cart_manager.confirm_sale(
    ...
    def create_form_layout_internal(self) -> ft.Container:
        ...
        def on_add(e: ft.ControlEvent):
            try:
                if ddl.value is None:
                    ...
                    raise ValueError(
                        "[POSController.on_add] Debe seleccionar un producto"
                    )
        ...
        def on_remove(index: int):
            try:
                if 0 <= index < len(self.cart):
                    self.cart.pop(index)
                    log.info(
                        f"[POSController.on_remove] Producto removido del carrito: index={index}"
                    )
                    refresh_cart()
        ...
        def on_confirm(e: ft.ControlEvent):
            if self.user_inv_id is None:
                ...
                log.error(
                    "[POSController.on_add] Intento de agregar al carrito sin usuario autenticado."
                )
                return None
            ...
            if created:
                ...
                log.error(
                    "[POSController.on_confirm] No se hizo el pago correctamente."
                )
                self.show_validate_error_dialog(["No se hizo el pago correctamente"])

        add_btn = ft.ElevatedButton("Agregar", on_click=on_add)
        confirm_btn = ft.ElevatedButton(
            "Confirmar venta", on_click=on_confirm, bgcolor=ft.Colors.GREEN_600
        )
    ...
    def update_content_internal(self):
        ...
        if isinstance(current, SaleSummaryStep):
            ...
            for cp in persisted:
                prod = (
                    getattr(cp, "product", None)
                    if not isinstance(cp, dict)
                    else cp.get("product")
                )
                raw_qty = (
                    getattr(cp, "quantity", None)
                    if not isinstance(cp, dict)
                    else cp.get("quantity", None)
                )
                ...
                if prod is None:
                    if self.page is not None:
                        self.show_error_dialog(
                            [f"Producto asociado al carrito no existe (cart_id={cid})"]
                        )
                    continue
                ...
                line = {
                    "product": prod,
                    "qty": qty,
                    "line_total": qty * int(getattr(prod, "price", 0) or 0),
                }
    ...
    def next_step_internal(self, e: Optional[ft.ControlEvent] = None):
        ...
        if isinstance(current, ProductSelectionStep):
            ...
            if pid and qty:
                prod = next(
                    (p for p in self.products if getattr(p, "product_id", None) == pid),
                    None,
                )
                if prod:
        ...
        if isinstance(current, CashPaymentStep):
            ...
            if created:
                log.info(
                    f"[POSController.next_step_internal] Pago realizado. Venta registrada con éxito: {created}"
                )
                ...
                log.error(f"[POSController.next_step_internal] ERROR: {created}")
    ...
    def create_steps_layout_internal(self) -> ft.Container:
        ...
        nav_buttons = ft.Row(
            [self.btn_prev, self.btn_next], alignment=ft.MainAxisAlignment.CENTER
        )
        ...
        return ft.Container(
            content=main_content, expand=True, padding=ft.Padding(20, 20, 20, 20)
        )

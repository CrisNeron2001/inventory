...
class SaleRegistrationsController:
    ...
    def on_load_sales(self) -> ft.Control:
        try:
            ...
            log.error(
                f"[SaleRegistrationsController.on_load_sales] Hubo un problema al cargar todos los ventas: {e}."
            )
            self.show_error_dialog(
                [f"Hubo un problema al cargar todos los ventas: {str(e)}."]
            )
            return ft.Container()

    def create_table_layout(self) -> ft.Container:
        loaded_sales = self.on_load_sales()
        return ft.Container(content=loaded_sales, expand=True)

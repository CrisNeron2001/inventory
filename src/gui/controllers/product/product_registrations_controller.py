...
class ProductRegistrationsController:
    ...
    def on_load_products(self) -> ft.Control:
        try:
            ...
            product_controls = self.product_info_table.create_controls(
                info_data=products
            )
            return ft.Column(controls=product_controls, expand=True)
        except Exception as e:
            log.error(
                f"[ProductRegistrationsController.on_load_products] Hubo un problema al cargar todos los productos: {e}."
            )
            self.show_error_dialog(
                [f"Hubo un problema al cargar todos los productos: {str(e)}."]
            )
            return ft.Container()

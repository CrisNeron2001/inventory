...
class EditProductFormController:
	def __init__(self, product_id: int, page: ft.Page, on_saved: Optional[Callable[[], None]] = None) -> None:
		...
		if product:
			...
			log.error(f"[EditProductFormController.__init__] No se pudo cargar el producto {product_name}")
			self.show_error_dialog([f"No se pudo cargar el producto {product_name}"])

	def on_submit(self, form_data: dict):
		is_valid, errors = self.validators.validate_edit_product_form_data(form_data)
		if not is_valid:
			log.error(f"[EditProductFormController.on_submit] Error al validar el formulario: {errors}")
		...
		if product_edited:
			log.info(f"[EditProductFormController.on_submit] Producto editado: {getattr(product_edited, 'name', '')}.")
			self.show_success_dialog(f"Producto editado: {getattr(product_edited, 'name', '')}.")
			return product_edited
		else:
			log.error("[EditProductFormController.on_submit] Error al editar producto (posible SKU duplicado)")
			sku_val = form_data.get("sku", "")
			if sku_val:
				sku_conflict_dialog(self.page, sku_val)
			else:
				log.error("[EditProductFormController.on_submit] No se pudo editar el producto. Intente nuevamente.")
				self.show_validate_error_dialog(["No se pudo editar el producto. Intente nuevamente."])

...
class AddProductFormController:
    ...
    def on_submit(self, form_data: dict):
        ...
        is_valid_av, errors_av = self.validator.validate_category_brand_data(form_data)
        errors: list[str] = []
        ...
        if errors:
            log.error(
                f"[AddProductFormController.on_submit] Error de validación: {errors}"
            )
        ...
        if isinstance(cat_val, dict):
            ...
            try:
                category_id = (
                    int(category_id_raw) if category_id_raw not in (None, "") else 0
                )
            except (ValueError, TypeError):
                category_id = 0
            category_dto = CategoryDTO(
                category_id=category_id, name=cat_val.get("name", "")
            )
        ...
        if isinstance(brand_val, dict):
            ...
            try:
                ...
                brand_id = 0
            brand_dto = BrandDTO(brand_id=brand_id, name=brand_val.get("name", ""))
        ...
        if product_added:
            log.info(
                f"[AddProductFormController.on_submit] Producto creado: {getattr(product_added, 'name', '')}."
            )
            self.show_success_dialog(
                f"Producto creado: {getattr(product_added, 'name', '')}."
            )
        else:
            log.error(
                "[AddProductFormController.on_submit] Error al crear producto (posible SKU duplicado)."
            )
            ...
            if sku_val:
                ...
                log.error(
                    "[AddProductFormController.on_submit] No se pudo crear el producto. Intente nuevamente."
                )
    ...
    def next_step(self, e):
        ...
        elif getattr(current_step, "form_id", "") == "category_brand":
            is_valid, errors = self.validator.validate_category_brand_data(step_data)
        else:
    ...
    def update_nav_buttons(self) -> None:
        ...
        if self.btn_next is not None:
            ...
            self.btn_next.on_click = (
                (lambda e: self.on_submit(self.form_data))
                if is_last
                else self.next_step
            )
    ...
    def create_layout(self) -> ft.Container:
        ...
        main_content = ft.Row(
            [
                ...
                ft.Container(
                    content=ft.Column(
                        [self.content_container, ft.Divider(height=20), nav_buttons]
                    ),
                    expand=True,
                    padding=ft.Padding(20, 20, 20, 20),
                ),

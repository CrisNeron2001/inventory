...
class UserRegistrationsController:
	...
	def on_load_users(self) -> ft.Control:
		try:
			...
			log.error(f"[UserRegistrationsController.on_load_users] Hubo un problema al cargar todos los usuarios: {e}.")
			self.show_error_dialog([f"Hubo un problema al cargar todos los usuarios: {str(e)}."])
			return ft.Container()

	def create_table_layout(self) -> ft.Container:
		loaded_users = self.on_load_users()
		return ft.Container(
            content=loaded_users,
            expand=True
        )

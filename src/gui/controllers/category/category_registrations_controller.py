...
class CategoryRegistrationsController:
	...
	def on_load_categories(self):
		try:
			...
			log.error(f"[CategoryRegistrationsController.on_load_categories] Hubo un problema al cargar todas las categorias: {e}.")
			self.show_error_dialog([f"Hubo un problema al cargar todas las categorias: {str(e)}."])

	def create_table_layout(self) -> ft.Container:
		loaded_categories = self.on_load_categories()
		return ft.Container(
            content=loaded_categories,
            expand=True
        )

...
class CategorySummaryStep:
	...
	def create_controls(self, form_data: Optional[Dict[str, Any]] = None) -> List[ft.Control]:
		...
		if self.error:
			controls.append(ft.Text(f"Error al importar categorías: {self.error}", color=ft.Colors.RED_400))
		else:
			controls.append(ft.Text(f"Categorías importadas: {len(self.categories)}", color=ft.Colors.GREEN_400))
			controls.append(self.create_table(self.categories, "Categorías"))

		total_pages = max(1, (len(self.categories) + self.page_size - 1) // self.page_size)
		pag_controls = []
		pag_controls.append(
			ft.ElevatedButton(
				text="Anterior",

	...
	def create_controls(self, form_data: dict) -> list[ft.Control]:
		...
		if hasattr(self, 'parent') and self.parent:
			for step in getattr(self.parent, 'steps', []):
				if isinstance(step, AvailabilityStep):
					prev_step = step
					break
		if prev_step and hasattr(prev_step, 'field_category') and prev_step.field_category:
		...
		if ctg_name == "N/A":
			if isinstance(category, dict):
				...
				ctg_name = getattr(category, "name", None) if category is not None else "N/A"
			if not ctg_name:
				ctg_name = "N/A"
		if brand_name == "N/A":
			if isinstance(brand, dict):
				...
				brand_name = getattr(brand, "name", None) if brand is not None else "N/A"
			if not brand_name:
				brand_name = "N/A"

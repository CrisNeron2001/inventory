...
class BrandInfoTable(Info, Paginator):
    def __init__(self, page):
        Info.__init__(self, "Marcas", "brand_info_table")
        columns = [
    ...
    def create_controls(self, info_data: Any) -> list[ft.Control]:
        ...
        rows: list[ft.DataRow] = [
            self.create_data_row(c) for c in self._get_paged_items()
        ]
                    ...
                    ft.Row(
                        [
                            ft.IconButton(
                                ...
                                on_click=lambda e, bid=data.get("brand_id", 0): (
                                    self.on_edit(bid)
                                ),
                            ),
                        ],
        ...
        return ft.Container(
            content=ft.Column(
                [
                    ...
                    ft.Text(
                        "Agrega marcas para verlos aquí",
                        size=14,
                        color=ft.Colors.GREY_500,
                        text_align=ft.TextAlign.CENTER,
                    ),
                    ft.ElevatedButton(
                        text="Agregar marca", icon=ft.Icons.ADD, on_click=self.on_add()
                    ),
                ],
    ...
    def get_data(self) -> dict:
        return {"brands": self.seq_data, "total": self._filtered_total()}

...
class CartDAO:
	...
	def create_cart_product_with_decrement(self, cart: CartProduct) -> Optional[CartProduct]:
		...
		try:
			...
			if not cp_row:
				log.error("No se obtuvo fila creada en insert_cart_product")
			...
			if not dec_row:
				self.db_conn.rollback()
				log.warning(f"No hay stock suficiente para product_id={dec_product_id}")
				return None
	...
	def remove_products_by_cart(self, cart: CartProduct) -> Optional[CartProduct]:
		...
		for pid in pids_list:
			...
			try:
				...
				log.info("Cursor no soporta fetchone tras DELETE")

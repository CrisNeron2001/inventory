...
class BrandDAO:
    ...
    def create_brand(self, brand: Brand) -> Optional[Brand]:
        if self.cursor and self.db_conn:
            log.info("[BrandDAO.created_brand] Creando una nueva marca.")
            self.cursor.execute(insert_brand, (brand.name,)) 
            self.db_conn.commit() 
            row: Any = self.cursor.fetchone() 
            brand_created = row_to_entity(row=list(row))
            log.info(f"[BrandDAO.created_brand] Nueva categoria creada: {brand_created}")
            return brand_created
        else:
            return log.error("[BrandDAO.created_brand] Error al crear una marca.")
    
    def get_brand_by_id(self, brand_id: int) -> Optional[Brand]:
        if self.cursor and self.db_conn:
            log.info("[BrandDAO.get_brand_by_id] Obteniendo por el id de marca.")
            self.cursor.execute(select_brand_name_by_id,(brand_id,)) 
            row: Any = self.cursor.fetchone() 
            getted_brand_id = row_to_entity(row=list(row)) 
            log.info(f"[BrandDAO.get_brand_by_id] Obtenido por el id de marca: {getted_brand_id}.")
            return getted_brand_id
        else:
            return log.error("[BrandDAO.get_brand_by_id] Error al obtener por el id de marca.")
    
    def get_all_brands(self) -> List[Brand]:
        if self.cursor and self.db_conn:
            log.info("[BrandDAO.get_all_brands] Obteniendo todas las marcas.")
            ...
            log.info(f"[BrandDAO.get_all_brands] Todas las marcas obtenidas: {brands}.")
            return brands
        else:
            log.error("[BrandDAO.get_all_brands] Error al obtener todas las marcas.")
            return []
    
    def edit_brand(self, brand: Brand) -> Optional[Brand]:
        if self.cursor and self.db_conn:
            log.info("[BrandDAO.edit_brand] Editando una marca.")
            self.cursor.execute(
				update_brand,
				(
					brand.name,
            ...
            log.info(f"[BrandDAO.edit_brand] Marca editada: {brand_updated}.")
            return brand_updated
        else:
            return log.error("[BrandDAO.edit_brand] Error al editar una marca.")
    
    def delete_brand(self, brand_id: int) -> bool:
        if self.cursor and self.db_conn:
            log.info("[BrandDAO.delete_brand] Eliminando una marca.")
            self.cursor.execute(delete_brand, (brand_id,))
            self.db_conn.commit()
            brand_deleted = self.cursor.rowcount > 0 
            log.info(f"[BrandDAO.delete_brand] Marca eliminada: {brand_deleted}.")
            return brand_deleted
        else:
            log.error("[BrandDAO.delete_brand] La marca no existe o ha sido eliminada.")
            return False
...
class ProductDAO:
    ...
    def create_product(self, product: Product) -> Optional[Product]:
        if self.cursor and self.db_conn:
            ...
            try:
                ...
                log.info(
                    f"[ProductDAO.create_product] Producto creado con éxito: {product_created}."
                )
                return product_created
            except errors.UniqueViolation:
                self.db_conn.rollback()
                log.error(
                    f"[ProductDAO.create_product] SKU duplicado al crear producto: {product.sku}."
                )
                return None
            except Exception as ex:
                self.db_conn.rollback()
                log.error(
                    f"[ProductDAO.create_product] Error al crear un producto: {ex}."
                )
                return None
        else:
    ...
    def get_product_by_id(self, product_id: int) -> Optional[Product]:
        if self.cursor and self.db_conn:
            log.info(
                "[ProductDAO.get_product_by_id] Obteniendo por el id del producto."
            )
            self.cursor.execute(select_product_by_id, (product_id,))
            row: Any = self.cursor.fetchone()
            getted_product_id = row_to_entity(list(row))
            log.info(
                f"[ProductDAO.get_product_by_id] Producto por id obtenido: {getted_product_id}."
            )
            return getted_product_id
        else:
            return log.error(
                "[ProductDAO.get_product_by_id] Error al obtener por el id del producto."
            )
    ...
    def get_all_products(self) -> List[Product]:
        if self.cursor and self.db_conn:
            ...
            log.error(
                "[ProductDAO.get_all_products] Error al traer todos los productos."
            )
            return []
    ...
    def edit_product_stock_by_name(self, name: str) -> Optional[Product]:
        if self.cursor and self.db_conn:
            ...
            log.info(
                f"[ProductDAO.edit_product_stock_by_name] Disponibilidad de producto editado: {product_stock_updated}."
            )
            return product_stock_updated
        else:
            return log.error(
                "[ProductDAO.edit_product_stock_by_name] Error al editar la disponibilidad del producto."
            )
    ...
    def delete_product(self, product_id: int) -> bool:
        if self.cursor and self.db_conn:
            ...
            log.info(
                f"[ProductDAO.delete_product] Producto eliminado: {product_deleted}."
            )
            return product_deleted
        else:
            log.error(
                "[ProductDAO.delete_product] El producto no existe o ya fue eliminado."
            )
            return False

...
class DatabaseConnection:
    ...
    def create_connection_db(cls):
        try:
            log.info(
                "[DatabaseConnection.create_connection_db] Conectando base de datos."
            )
            db_conn = connect(
                host=os.environ.get("PG_HOST") or "localhost",
                port=os.environ.get("PG_PORT") or "5432",
                user=os.environ.get("PG_USER") or "postgres",
                password=os.environ.get("PG_PASSWORD") or "neron098",
                database=os.environ.get("PG_DATABASE") or "invdb",
            )
            cls.db_conn = db_conn
            log.info(
                f"[DatabaseConnection.create_connection_db] Conección exitosa: \n{db_conn}."
            )
            return db_conn
            ...
            if "authentication" in error_msg:
                log.error(
                    "[DatabaseConnection.create_connection_db] Credenciales de la base de datos incorrectas: %s",
                    e,
                )
                raise DatabaseFailure("non-retryable") from e
            else:
                log.error(
                    "[DatabaseConnection.create_connection_db] Error operacional: %s", e
                )
                raise ConnectionFailure("non-retryable") from e
        except Exception as e:
            log.critical(
                " [DatabaseConnection.create_connection_db] Un error desconocido: %s", e
            )

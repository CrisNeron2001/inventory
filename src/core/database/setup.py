...
class DatabaseSetup:
    ...
    def execute_create_entities(self):
        try:
            log.info("[DatabaseSetup.execute_create_entities] Procedimiento de creación de tablas.")
            if self.db_conn and self.cursor:
                ...
                self.conn.commit()
                log.info("[DatabaseSetup.execute_create_entities] Todas las tablas han sido creadas exitosamente.")
            else:
                log.error("[DatabaseSetup.execute_create_entities] La conexión hacia la base de datos no ha sido establecida.")
        except Exception as e:
            log.critical("[DatabaseSetup.execute_create_entities] Un error desconocido ha ocurrido durante el procedimiento: %s", e)
    ...
    def execute_check_entities(self):
        try:
            log.info("[DatabaseSetup.execute_check_entities] Procedimiento de comprobar tablas existentes.")
            if self.cursor:
                ...
                if missing:
                    log.info("[DatabaseSetup.execute_check_entities] Faltan tablas: %s. Procediendo a crearlas.", ", ".join(missing))
                    self.execute_create_entities()
                else:
                    log.info("[DatabaseSetup.execute_check_entities] Las tablas ya existen.")
            else:
                log.error("[DatabaseSetup.execute_check_entities] La conexión hacia la base de datos no ha sido establecida.")
        except Exception as e:
            log.critical("[DatabaseSetup.execute_check_entities] Un error desconocido ha ocurrido durante el procedimiento: %s.", e)
            raise PermanentFailure("non-retryable") from e
        finally:
